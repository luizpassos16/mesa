# 02 — Diagnóstico da MESA (onde estamos hoje)

> Síntese de dados de Granola (24 reuniões, 28/abr–27/mai), Tally (~15 formulários-chave) e Drive (8 decks + briefing). Maio/2026.
> Serve de base factual para o benchmark (03) e a estratégia (04).

## A. Identidade e posicionamento

A MESA se posiciona como **criadora de ambientes (ambiência) para empresários e líderes** que valorizam conversas relevantes, ambiente seguro e relações consistentes — ancorada na **hospitalidade mineira**.

Narrativa-chave (decks): *"A Mesa não é sobre comida. A hospitalidade é só a linguagem que gera oportunidades. Não somos um jantar — somos um Board."* Tagline: **"A Mesa não é onde você aprende mais. É onde você decide melhor."**

**Três pilares** (consistentes nos decks): **Clareza** ("o foco não é aprender mais, é enxergar melhor") · **Confiança** (espaço seguro, curado, confidencial — "apenas pares") · **Decisão** ("conversas que terminam em decisões assumidas, não em insights").

Gancho emocional: a **"solidão estratégica"** do empresário de médio porte — "grande demais para improvisar, pequeno demais para ter um board corporativo completo".

## B. Frentes de produto e estado atual

### Comunidade / Board (núcleo de receita)
- **Oferta hoje: SEM oferta vigente.** Última leva vendida a **R$ 14.997/cadeira × 12 cadeiras (≈ R$ 180k)** (confirmado 27/05). Hoje é "praça limpa" para desenhar a próxima oferta.
- Boards íntimos de **10–12 empresários** (decks institucionais às vezes citam 15 — **inconsistência a resolver**), faturamento até ~R$ 5–10M/ano, sócios/fundadores.
- Entregas por ciclo: Board Meetings (formato conselho, hot seat, conselheiros convidados), Sessões de Decisão Estratégica (1 empresa por vez), Open Company, diagnóstico individual de entrada, monitorias mensais, canal no WhatsApp.
- Funil por **curadoria**: "não compre, aplique para uma entrevista" → análise de fit → convite. *"A curadoria é o produto."*
- Prova social forte: Pierre Berenstein (Outback), Gustavo Caetano (SambaTech), Marcelo Marques (Mobly), Romeu Zema, Paulo Camargo (ex-McDonald's), David Ledson (mentor G4), entre outros.
- **NPS da Turma 1 ≈ 100.**

### Mesa IA (maior tração de curto prazo)
- Produto definido com **César + Rodrigo**: aulas semanais + grupo WhatsApp, **R$ 5.000**, **lançamento final de junho/2026**.
- Funil de lançamento: **grupo → aulão → venda**.
- Eventos de entrada: imersão "DNA" (13/05) → evento prático "mãos no teclado" (20–21/05), ~R$ 490–497.
- Posicionamento: **desmistificar a "IA mágica"**; foco em automações reais (MCP, Claude + Obsidian, conciliação, agentes, RAG) para PMEs — "oceano azul", analogia com Conta Azul há 10–15 anos.
- Divisão: César (modelos mentais/casos), Rodrigo (ferramentas/metodologia), Luiz (ecossistema/relacionamento).

### Mesa Soluções (em estruturação)
- Modelo guarda-chuva: a Mesa capta leads, parceiros executam **sob a marca Mesa**.
- Migrando de **comissão por projeto → mensalidade fixa (R$ 2.000/mês) + reunião mensal**.
- Áreas e parceiros: Eventos (Ana/Wallace), Redução de custos (Igor — projetos R$ 5–15k, % de indicação), Financeiro (Giovanni), Administrativo (Tiago), Restaurantes (Adriana/Hana), Tributário (Vitinho).

### Mesa Experience (eventos/ambiência)
- Marca das experiências presenciais (grupos reduzidos), na **BE HOUSE** (ecossistema BE GROUP).
- Menu de catering 2026: **R$ 325 / R$ 350 / R$ 380 por pessoa** + add-ons.

## C. Quem é o público (dados Tally)

- **Geografia hiperconcentrada em BH/RMBH.** A Mesa é hoje, na prática, um **produto BH**.
- **Cargo:** dominância de sócio/fundador e C-level. Quase todos empreendem.
- **Porte (distribuição "barbell"):** muita base pequena (~52% até R$ 500k/mês) + núcleo premium robusto (**~24% acima de R$ 5M/mês** → alvo do Board).
- **Setores:** Tecnologia, Saúde, Alimentação/Bebidas, Eventos, Marketing, Educação, Jurídico, Construção, Financeiro.
- **O que buscam:** gerar negócios, ampliar relacionamento, parceiros estratégicos, sair do operacional. Motivação dominante = **networking de elite / conexões genuínas**.
- **Dores transversais (nº 1):** escala/crescimento e **vendas/captação qualificada**; depois gestão de pessoas, processos, fluxo de caixa. Verbatim recorrente: *"ultrapassar a barreira dos 7 dígitos mensais"*.

## D. Demanda por produto (sinais quentes)

- **IA — a demanda migrou de "o que é" para "me ajuda a implementar".** O público já usa IA ("ativa" ou "testando"); a frustração é a **última milha**: deploy, integração, RAG, hospedar para clientes. → janela clara para **Mesa Soluções / projetos sob medida**.
- **Board — pipeline aquecido:** Lista de Espera com timing majoritário "imediato / 2º sem 2026"; **100% disponíveis** para encontros mensais à noite em BH.
- **Recorrência validada:** maioria toparia um "clube com encontros mensais por 6 meses" → valida modelo de **assinatura/comunidade paga**.
- **Aceleração comercial:** "O que você precisa para 2026" aponta **crescimento/escala de vendas** como prioridade nº 1, e **"a operação me engole"** como maior trava → fio condutor de conteúdo do semestre.

## E. Satisfação e críticas acionáveis

- **NPS saturado em 10** em praticamente todos os produtos (A Mesa geral, Boards, Mesa Aberta·IA). O produto é amado — o desafio **não é satisfação, é escala e consistência**.
- **Críticas recorrentes (poucas e claras):**
  1. **Local/venue inconsistente** (Mesa Aberta·IA fora do padrão; sem estacionamento) — padronizar o ambiente.
  2. **Gestão de tempo/mediação** nos Boards ("tempo estourou", "falta mediação") — apertar a facilitação.
  3. Pedido de **mais dinâmica/circulação** (rodadas de negócio, conexão prévia via Instagram).
- **Pedido nº1 sobre IA:** mais prática e **aprofundar em agentes** ("pode ser uma parte 2").

## F. Aquisição

- **Instagram é o canal de descoberta dominante**, seguido de **indicação de membro**.
- Já existe estrutura de referral embrionária (Banco de Indicações, n=35) — **formalizar referral pode destravar volume**.

## G. Parcerias mapeadas

- **One (investimentos):** ativa. Remuneração nova = R$ 1.500 por milhão captado, na entrada. Evento conjunto (impostos/taxas) previsto para 2ª quinzena de junho. Pendência: cláusula de não concorrência.
- **Brendi + Hacoo/Hanna (restaurantes):** SaaS de WhatsApp (6.500 clientes, 92 em BH) quer eventos proprietários + comunidade. Mesa/Hacoo oferecem metodologia 360° de eventos; BH como praça de teste. Hacoo propõe duas opções de investimento (até R$ 300–400k), parcerias Catupiry/Coca-Cola.
- **MVP, CRIE (70 empresários cristãos), Raco/Ana (formulário inteligente), Fortini (ONG/embaixadores), Wallace França (sociedade em eventos sociais).**

## H. O sistema de dados em construção

Confirma a tese (doc 00):
- Sistema atual roda no **Lovable** (`mesaadvisory.lovable.app`, provável backend Supabase) — **em uso hoje**, não descartado; direção de longo prazo é **infra própria / controle total dos dados**.
- Sistema **multiusuário** com permissões (admin x membro) e upload de material bruto.
- Pilar **Clareza** = formulários progressivos + IA = motor de captura.
- **Gargalo:** os dados ainda vivem em silos (Granola, Tally, Drive, Notion); a "pasta por pessoa" unificada (CPF/WhatsApp/e-mail) ainda não existe.

## I. Pontos a confirmar antes de fechar o plano

1. ~~**Oferta vigente do Board**~~ → **RESOLVIDO (27/05):** não há oferta vigente hoje. Última leva = **R$ 14.997 × 12 cadeiras (≈ R$ 180k)**. Os outros valores nos decks (R$ 16.997/19.997, 15 cadeiras) são versões antigas/aspiracionais — a referência real é a última venda.
2. **Preço do Board nas reuniões** apareceu como R$ 15.000 e R$ 20.000/semestre — alinhar com os decks.
3. **G4 como benchmark:** aparece só como prova social (David Ledson) e âncora de preço — não há análise do modelo. Definir o que replicar (ver doc 03).
4. **tl;dv:** acesso a confirmar.
5. **Higienização do Tally:** remover respostas-teste antes de usar números como meta.
6. ~~**Ferramenta de captura atual**~~ → **RESOLVIDO (27/05):** o sistema em uso é o app **Lovable `mesaadvisory.lovable.app`** (provável backend Supabase) — não foi descartado. O pensamento bruto do Luiz vive em Notion + chats. Próximo passo: inventariar o banco real e organizar o que está solto.
